...
const summary = (): Summary => ({
  ...
  settings: {
    ...
    water_balance_min_mm: 5,
    water_balance_max_event_mm: 10,
    water_balance_max_rain_credit_mm: 15,
  ...
  weather: {
    ...
    et_wind_factor: 1.07,
    average_humidity_percent: 63,
    et_humidity_factor: 0.96,
    et_reference_mm: 5,
        ...
        programs: [
          {
            ...
            total_minutes: 71,
            water_balance_before_mm: 6.2,
            daily_water_need_mm: 4.7,
            daily_effective_rain_mm: 1.2,
            daily_ledger_rain_mm: 1,
            forecast_rain_mm: 0.8,
            forecast_ledger_rain_mm: 0.7,
            irrigation_target_mm: 8,
            remaining_balance_mm: 1.7,
            water_balance_gap_days: 2,
            water_balance_backfilled_gap_days: 2,
            water_balance_unaccounted_gap_days: 0,
            water_balance_rebaselined_after_gap: false,
            water_balance_last_rebaseline_date: null,
        ...
        programs: [
          {
            ...
            planning_status: "smart_planned",
            selection_reason:
              "Ha öntözés válik szükségessé, 04:45 a legkedvezőbb időpont.",
            status: "water_need_deferred",
            reason:
              "A várható eső csökkenti a vízhiányt, ezért most nem öntöz.",
            total_minutes: 0,
            water_balance_before_mm: 2.1,
            daily_water_need_mm: 3.4,
            daily_effective_rain_mm: 0.2,
            forecast_rain_mm: 4.2,
            irrigation_target_mm: 0,
            remaining_balance_mm: 1.3,
            zones: plannedZones(morning).map((zone) => ({
              ...zone,
              planned_minutes: 0,
            })),
        ...
        programs: [
          {
            ...
            selection_reason:
              "A legkisebb, 0,5 mm-es kijuttatás teljes zónasora sem fér bele a 60 perces időablakba.",
            status: "smart_no_fit",
            reason:
              "A zónasort nem vágja félbe; a fennmaradó vízhiány megmarad a következő alkalomra.",
            total_minutes: 71,
            zones: plannedZones(morning),
            weather,
          },
...
const hourlyForecast = (): HourlyForecast => {
  ...
  const hours = Array.from({ length: 36 }, (_, index) => {
    ...
    return {
      ...
      humidity_percent: rainy ? 86 : Math.max(42, 78 - (localHour - 6) * 2),
      condition: rainy
        ? "rainy"
        : night
          ? "clear-night"
